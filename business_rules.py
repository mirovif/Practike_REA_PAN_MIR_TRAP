...
def get_purchase_rhythm(user_id: int) -> dict:
    """AVG/MIN/MAX дней между заказами для пользователя."""
    query = """
...
# ── Сервис 2: Товары, которые вы забыли ──────────────────────────────────────

def get_forgotten_products(user_id: int, last_n: int = 3) -> list[dict]:
    """Товары, купленные ≥3 раз исторически, но отсутствующие в последних N заказах."""
    with _get_conn() as conn:
        # последние N order_id
        recent_ids = [
        ...
        query = f"""
            ...
            LIMIT 10
        """
        rows = conn.execute(query, (user_id, *recent_ids)).fetchall()
...
def ask_mistral_forgotten(user_id: int, products: list[dict]) -> str:
    ...
    items = "\n".join(
        f"- {r['product_name']} (категория: {r['aisle']}, куплено раз: {r['total_bought']})"
        for r in products
    )
    user_message = (
...
def get_best_order_time(user_id: int) -> dict:
    """Самый частый день недели и час для заказов пользователя."""
    query = """
    ...
    with _get_conn() as conn:
        ...
        heatmap_rows = conn.execute(heatmap_query, (user_id,)).fetchall()
